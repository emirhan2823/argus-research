...
def run(args):
    ...
    try:
        ...
        if getattr(args, 'download_binance', False):
            ...
            print(f"Downloaded -> {os.path.join(args.data_dir, args.symbol + '.csv')}")
            ...
            trade_cooldown_counter -= 1
             ...
             if verdict.decision == "GO":
                 if is_lockdown:
                     ...
                     final_risk = effective_max_risk * mode_snap.risk_multiplier
                     effective_leverage = min(mode_snap.leverage_allowed, effective_lev_cap)
                     
                     final_risk = effective_max_risk * mode_snap.risk_multiplier
                     effective_leverage = min(mode_snap.leverage_allowed, effective_lev_cap)
                     
                     # Edge Score Logic (Moved Up)
                     # edge_score = 0.0
                     # slope_raw = ae_vote.metadata.get('slope', 0.0)
                     # adx = or_vote.metadata.get('adx', 0.0)
                     # slope_bps = (abs(slope_raw) / current_bar.close) * 10000.0
                     # adx_bps = adx * 0.4
                     # edge_score = (0.6 * slope_bps) + (0.4 * adx_bps)
                     
                     if verdict.decision == "GO":
                         go_scores.append(edge_score)


                          # --- NEW FILTERS (Phase 2 & 3 & 5) ---
                          
                          # 1. Min ADX Filter (V5 AdxGate) - Production Standard
                          min_adx = getattr(args, 'min_adx', 45.0)
                          curr_adx = or_vote.metadata.get('adx', 0.0)
                          
                          # Log Production Guards
                          if not getattr(args, 'quiet', False):
                               print(f"[{current_bar.dt}] GUARD CHECK | Regime:{regime} ADX:{curr_adx:.1f} Gate:{'FAIL' if curr_adx < min_adx else 'PASS'}")
                         ...
                         # Logic: If ATR Spike -> Check if Trend Quality is bad (Low ADX or Low Slope)
                         vt_mult = getattr(args, 'vol_trap_mult', 0.0) # V3 Legacy
                         vt_thresh = getattr(args, 'vol_trap_threshold', 0.0) # V2 New
                         
                         # Use either legacy or new. If both, specific takes precedence? 
                         # Let's say Mult is the "Threshold".
                         ...
                         if effective_vt_thresh > 0.0 and len(atr_history) >= 10:
                             ...
                             if atr_avg > 0 and (atr_curr / atr_avg) > effective_vt_thresh:
                                  ...
                                  # Check Allow Conditions (Smart Filter)
                                  if getattr(args, 'vol_trap_v2', False):
                                      # Conditional Reject
                                      # Allow if: ADX > 25 AND AbsSlope > 2.0 (Strong Trend)
                                      ...
                                      if not is_strong_trend:
                                           reporter.log_reject(current_bar.timestamp, args.symbol, "REJECT_VOL_TRAP_V2", f"ATR {atr_curr:.2f} > {effective_vt_thresh}x. Weak Trend (ADX:{adx_v:.1f} Sl:{slope_v:.1f})")
                                           continue
                                  else:
                                      # Hard Reject (V3 Legacy)
                                      reporter.log_reject(current_bar.timestamp, args.symbol, "REJECT_VOL_TRAP", f"ATR {atr_curr:.2f} > {effective_vt_thresh}*{atr_avg:.2f}")
                                      continue
                         ...
                         # Requires 'expected_move' to be calculated via 'verdict.metadata'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_exp_move_bps", type=float, default=80.0, help="Reject signals with ExpMove > N bps")
    parser.add_argument("--vol_trap_mult", type=float, default=0.0, help="Reject if ATR > Mult * AvgATR(20) (Legacy)")
    parser.add_argument("--vol_trap_threshold", type=float, default=0.0, help="VolTrap Threshold (V2)")
    parser.add_argument("--vol_trap_v2", action="store_true", help="Enable VolTrap V2 (Smart Conditional Filter)")
    parser.add_argument("--min_adx", type=float, default=45.0, help="Reject signals if ADX < N (Weak Trend)")

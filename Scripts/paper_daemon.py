...
class PaperDaemon:
    ...
    def append_decision(self, bar, verdict, router_res, score, exp_move, adx):
        ...
        row = f"{ts_iso},{bar_ts_iso},{self.cfg['args'].symbol},{verdict.regime},{router_res['mode_final']},{verdict.decision},{verdict.direction},{score:.2f},{exp_move:.1f},{adx:.1f},{reasons}\n"
    ...
    def append_reject(self, bar, code, detail):
        ...
        row = f"{ts_iso},{bar_ts_iso},{self.cfg['args'].symbol},{code},{detail}\n"
    ...
    def process_bar(self, bar: Bar):
        ...
        if verdict.decision == "GO":
             ...
             atr_base = atr_bps * args.atr_k
             adx_factor = 0.0
             if adx_m > 20: adx_factor = min(1.0, (adx_m - 20) / 40.0)
             adx_boost = 1.0 + adx_factor
             
             expected_move = max(slope_bps_abs, atr_base) * adx_boost * args.adx_boost
             cap_bps = atr_bps * args.exp_cap_mult
        ...
        elif router_res['policy'] == "defense_flat": r_risk_mult = 0.0
        
        if verdict.decision == "GO" and r_risk_mult == 0.0:
            verdict.decision = "BLOCK"
            self.reporter.log_reject(bar.timestamp, args.symbol, "ROUTER_DEFENSE", "Risk=0.0")
        ...
        if verdict.decision == "GO" and adx_m < min_adx:
            verdict.decision = "BLOCK"
            self.reporter.log_reject(bar.timestamp, args.symbol, "MIN_ADX", f"{adx_m:.1f} < {min_adx}")
            self.append_reject(bar, "MIN_ADX", f"{adx_m:.1f} < {min_adx}")

        # Max Exp
        max_exp = args.max_exp_move_bps
        em_check = verdict.metadata.get('expected_move', 0.0)
        if verdict.decision == "GO" and em_check > max_exp:
            verdict.decision = "BLOCK"
            self.reporter.log_reject(bar.timestamp, args.symbol, "MAX_EXP", f"Exp {em_check:.1f} > {max_exp}")
        ...
        # 5. Execution
        self.broker.mark_to_market(args.symbol, bar.close)
        
        # Check Exits (Brackets)
        exit_fill = self.broker.check_brackets(args.symbol, bar.high, bar.low, bar.timestamp)
        if exit_fill:
